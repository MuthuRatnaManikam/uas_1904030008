...
require 'koneksi.php';
$dbuku = query("SELECT * FROM buku");

if (isset($_POST['cari'])) {
  $dbuku = cari($_POST['keyword']);
}
    ...
    <?php if (empty($dbuku)) : ?>
      <tr>
        <td colspan="4" class="alert alert-danger text-center" role="alert">
          <p><b>Mahasiswa Tidak itemukan</b></p>
        </td>
      </tr>
    <?php endif; ?>
    <tbody>
      <?php $no = 1; ?>
      <?php foreach ($dbuku as $cmb) : ?>
        <tr>
          <th scope="row"><?php echo $no ?></th>
          <td><?php echo $cmb['judul_buku']; ?></td>
          <td><?php echo $cmb['tahun_terbit']; ?></td>
          <td><?php echo $cmb['jumlah_buku']; ?></td>
          <td><?php echo $cmb['jenis_buku']; ?></td>
          <td><?php echo $cmb['kode_rak']; ?></td>
          <td>
            <a href="editbuku.php?id=<?= $dbuku['id'];  ?>" class="btn btn-warning" role="button">Edit</a> |
            <a href="hapus.php?id=<?= $dbuku['id'];  ?>" onclick="return confirm('apakah anda ingin menghapus data ini');" class="btn btn-danger" role="button">Hapus</a>
          </td>
        </tr>
        <?php $no++ ?>
      <?php endforeach ?>
